...
export default function CartPage() {
  ...
  const openRazorpay = (invoiceId: string, amount: number, orderId: string, subscriptionId: string) => {
    const script = document.createElement('script')
    script.src = 'https://checkout.razorpay.com/v1/checkout.js'
    script.async = true
    script.onload = () => {
      const rzp = new (window as unknown as { Razorpay: typeof Razorpay }).Razorpay({
        ...
        handler: async (response: { razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature: string }) => {
          try {
            ...
            router.push(`/orders/${subscriptionId}`)
          } catch {
        ...
        modal: {
          ondismiss: () => {
            toast.info('Payment cancelled. You can pay later from My Invoices.')
            router.push(`/orders/${subscriptionId}`)
          },
        },
      })
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Use invoice_id returned directly from the order response
      try {
        if (invoiceId) {
          const orderRes = await api.post('/api/payments/create-order', { invoice_id: invoiceId })
          const { order_id, amount } = orderRes.data.data
          openRazorpay(invoiceId, amount, order_id, subscriptionId)
        } else {
          router.push(`/orders/${subscriptionId}`)
        }
      } catch {
        toast.error('Failed to initiate payment. You can pay from My Invoices.')
        router.push(`/orders/${subscriptionId}`)
      }
    } catch (err: unknown) {
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      {item.plan_id && (
                        <Badge className="bg-indigo-100 text-indigo-700 text-xs mt-1">Monthly Plan</Badge>
                      )}
                      <p className="text-sm text-slate-600 mt-1">₹{item.unit_price.toLocaleString('en-IN')} each</p>
                    </div>
