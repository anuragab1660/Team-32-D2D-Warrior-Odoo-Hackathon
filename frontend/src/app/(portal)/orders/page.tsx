      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {filtered.map((sub, i) => {
            ...
            const linesCount = parseInt(String((sub as unknown as Record<string,unknown>).lines_count ?? sub.lines?.length ?? 0))
            const totalAmount = parseFloat(String((sub as unknown as Record<string,unknown>).total_amount ?? 0))
                  ...
                  <CardContent className="p-5">
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      {planName ? (
                        ...
                        <span className="text-sm font-medium text-slate-700">{productNames}</span>
                      ) : (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="text-xs text-slate-500">
                        {linesCount > 0 ? (
                          <span>{productNames || `${linesCount} product${linesCount !== 1 ? 's' : ''}`}</span>
                        ) : 'No products'}
                        {totalAmount > 0 ? ` · ₹${Number(totalAmount).toLocaleString('en-IN')}` : ''}
                      </div>
