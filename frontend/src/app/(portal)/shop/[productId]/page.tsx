...
import { ArrowLeftIcon, LoaderIcon, ShoppingCartIcon, ZapIcon, CheckCircleIcon } from 'lucide-react'
...
export default function ProductDetailPortalPage() {
  ...
  const basePrice = (product?.sales_price ?? 0) + (selectedVariantObj?.extra_price ?? 0)
        ...
        <div className="md:sticky md:top-24 self-start">
          <Card className="border-slate-200">
            <CardContent className="p-5 space-y-5">
              {/* Price */}
              <div>
                <p className="text-3xl font-bold text-indigo-700">₹{basePrice.toLocaleString('en-IN')}</p>
                {billingMode !== 'onetime' && (
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {(['monthly', 'weekly', 'yearly', 'daily'] as const).map(bp => {
                    const hasPlan = plans.some(p => p.billing_period === bp)
                    return (
                      <button
                        key={bp}
                        onClick={() => handleBillingModeChange(bp)}
                        disabled={!hasPlan}
                ...
                {selectedPlan && (
                  <p className="text-xs text-slate-400">{selectedPlan.name} — ₹{selectedPlan.price}/{selectedPlan.billing_period}</p>
                )}
              </div>
