        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-amber-800">
              Your subscription {(expiringSoon[0] as unknown as Record<string,string>).plan_name || (expiringSoon[0] as unknown as Record<string,string>).product_names || expiringSoon[0].subscription_number} expires in {daysUntil(expiringSoon[0].expiration_date!)} day{daysUntil(expiringSoon[0].expiration_date!) !== 1 ? 's' : ''}.
            </p>
          </div>
              ...
              <CardContent className="p-5">
                ...
                {pendingInvoices.length > 0 && (
                  <p className="text-xs text-amber-600 mt-1">
                    ₹{pendingInvoices.reduce((s, i) => s + Number(i.grand_total), 0).toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} due
                  </p>
                )}
              </CardContent>
            </Card>
          ...
          <motion.div custom={2} initial="hidden" animate="visible" variants={{ hidden: { opacity: 0, y: 12 }, visible: (i: number) => ({ opacity: 1, y: 0, transition: { delay: i * 0.05 } }) }}>
            <Card className="border-slate-200">
              <CardContent className="p-5">
                ...
                <p className="text-2xl font-bold text-slate-900">₹{Math.round(totalSpentThisYear).toLocaleString('en-IN')}</p>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <CardContent>
            <div className="space-y-4">
              {activeSubscriptions.slice(0, 3).map((sub, i) => {
                ...
                const planName = (sub as unknown as Record<string,string>).plan_name
                const billingPeriod = (sub as unknown as Record<string,string>).billing_period
                const productNames = (sub as unknown as Record<string,string>).product_names
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        {planName ? (
                          ...
                          <p className="text-sm font-semibold text-slate-800">{productNames}</p>
                        ) : null}
                        {isExpiringSoon && (
